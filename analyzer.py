...
class Analyzer:
				...
				for key,p in enumerate(chapter_content):
					...
					if p_type and p_type == "paragraph" and p_content:
						...
						if re.search(rf"\b{re.escape(goal_text)}\b", text): #goal_text
							...
							parag = key + 1
							print(text)
							ezro_format = f"( Т{volume_id}.Г{chapter_id}.П{parag} )"
	...
	def print_count_paragraphs_and_count_words(self):
		...
		print(f"Глава с минимальным кол. упоминании {Config.character_name_to_search} {result[rimuru_word["name_max"]]["volume"]} том {result[rimuru_word["name_max"]]["chapter"]} глава <{rimuru_word["name_min"]}> c {rimuru_word["min"]}")
		print(f"Глава с максимальным кол. упоминании {Config.character_name_to_search} {result[rimuru_word["name_max"]]["volume"]} том {result[rimuru_word["name_max"]]["chapter"]} глава <{rimuru_word["name_max"]}> c {rimuru_word["max"]}")
